 ...
 * Esta función mantiene la posición del cursor correctamente
 */
export function formatearRutEnInput(
  valor: string, 
  posicionCursor: number
): { valorFormateado: string; nuevaPosicion: number } {
  // Obtener el valor sin formato
  const valorLimpio = limpiarRut(valor);
  
  // Si está vacío, retornar vacío
  if (!valorLimpio) {
    return { valorFormateado: '', nuevaPosicion: 0 };
  }
  ...
  // Contar cuántos caracteres de formato se agregaron antes de la posición actual
  const caracteresAntesCursor = valor.slice(0, posicionCursor);
  const caracteresLimpiosAntesCursor = limpiarRut(caracteresAntesCursor);
  
  let nuevaPosicion = 0;
  let caracteresContados = 0;
  
  for (let i = 0; i < valorFormateado.length; i++) {
    const char = valorFormateado[i];
    
    if (char === '.' || char === '-') {
      // Es un caracter de formato, no contar
      nuevaPosicion++;
    } else {
      // Es un caracter del RUT
      caracteresContados++;
      nuevaPosicion++;
      
      if (caracteresContados >= caracteresLimpiosAntesCursor.length) {
        break;
      }
    }
  }
  
  return { valorFormateado, nuevaPosicion };
}
